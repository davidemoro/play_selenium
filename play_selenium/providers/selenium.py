...
class SplinterCommandProvider(BaseProvider):
    ...
    def _navigation(self):
        navigation = getattr(self.engine, '_navigation', None)
        if navigation is None:
            self.engine._navigation = navigation = self \
                .engine.request.getfixturevalue('navigation')
        return navigation

    def locator_translate(self, locator):
        """ Translates locator to splinter selector
            format
        """
    ...
    def command_get(self, command, **kwargs):
        """ get """
        if self._navigation.page is None:
            page = self._navigation.get_page_instance()
            self._navigation.setPage(page)
        self._navigation.page.driver_adapter.open(command['url'])
    ...
    def command_clickElement(self, command, **kwargs):
        ...
        self._navigation.page.find_element(*selector).click()
    ...
    def command_setElementText(self, command, **kwargs):
        ...
        selector = self.locator_translate(command['locator'])
        text = command['text']
        self._navigation.page.find_element(*selector).fill(text)
    ...
    def command_select(self, command, **kwargs):
        ...
        raw_element = self._navigation.page.find_element(
            *selector)._element
        raw_element.click()
        if text is not None:
    ...
    def command_waitForElementPresent(self, command, **kwargs):
        ...
        self._navigation.page.wait.until(_wait)
    ...
    def command_sendKeysToElement(self, command, **kwargs):
        ...
        selector = self.locator_translate(command['locator'])
        self._navigation.page.find_element(*selector) \
            ._element \
